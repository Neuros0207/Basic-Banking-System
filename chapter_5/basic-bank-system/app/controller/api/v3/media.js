...
module.exports = {
    ...
    uploadFile: (req, res)=>{
        const fileUrl = `${req.protocol}://${req.get('host')}/applications/${req.file.filename}`
    ...
    uploadText: (req, res)=>{
        const textUrl = `${req.protocol}://${req.get('host')}/texts/${req.file.filename}`
    ...
    qrGenerate : (req,res)=>{
        ...
        if(typeof req.body.string === 'object' && req.body.string !== null){
            ...
            qr_png = qr.image(req.body.string, {type: 'png', ec_level: 'H', size: 500})
        }
        ...
        qr_png.pipe(fs.createWriteStream(`./public/qr-images/${formattedTime}/qr-image-${currentTime}.png`))
        const qrUrl = `${req.protocol}://${req.get('host')}/qr-images/${formattedTime}/qr-image-${currentTime}.png`
        res.status(200).json({
            status: 'success',
