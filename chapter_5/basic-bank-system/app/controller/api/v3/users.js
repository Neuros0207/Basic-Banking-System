...
module.exports = {
    async getUsers(req, res){
        if(req.user.role !== 'Admin'){
            return res.status(401).json({
                status : 'failed',
                message : "you're not authorized to access this page! You need to be an admin",
                data: null
            })
        }
    ...
    async postUsers(req, res){
        try {
            ...
            result = await model.userRegistration(req.body.user_name, req.body.phone_number, req.body.identity_type,req.body.identity_number,req.body.address)
            if(result){
                res.status(201).json({
                status: 'success',
                code: 201,
    ...
    async putUsersById(req,res){
        try {
            ...
            if(!put_parameter.every(value => Object.keys(req.body).includes(value)) || (!+req.params.id)){
            return res.status(400).json({
                status : 'fail',
                code : 400,
                message : 'Bad request: request form incomplete or the id is not integer'
            })
            }
