...
module.exports = {
    async getAccounts(req,res){
        if(req.query){
            let result = await model.searchAccounts(req.query.page_number, req.query.display_limit, req.query.search)
            if(!result.length){
                res.status(200).json({
                    status : 'success',
                    code : 200,
    ...
    async postAccount(req, res){
        try {
            if(!req.body.email || !req.body.password || !+req.body.balance || !req.body.identity_number ){
            ...
            if(!search_id ){
                ...
                const result = await model.createNewAccount(req.body.balance,req.body.password,req.body.email, search_id.user_id, hashed_password)
               if(result){
